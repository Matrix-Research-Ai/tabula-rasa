    ...
    for ckpt_name in ['best.pt', 'final.pt', 'checkpoint.pt']:
        ...
        if ckpt_path.exists():
            try:
                ...
                cfg.n_heads = {128: 4, 64: 2, 96: 4, 256: 8}.get(ckpt_d_model, max(1, ckpt_d_model // 32))
    ...
    for ckpt_name in ['checkpoint.pt', 'best.pt']:
        ...
        if ckpt_path.exists():
            try:
                ...
                ckpt_L = len([k for k in sd if k.startswith('layers.') and k.endswith('.attention.wq.weight')])
...
def quick_eval_all(ops, tokenizer, num_problems=30):
    ...
    for op in ops:
        ...
        if stored_acc is not None and stored_acc > 0:
            ...
            print(f'  {OP_NAMES[op]:>14}: {stored_acc:.1f}% (cached) | d={cfg.d_model} L={cfg.n_layers}')
        else:
            # Run quick eval
            tok.max_seq_len = cfg.max_seq_len
            acc = evaluate(model, tok, cfg, op, num=num_problems)
            results[op] = acc
            print(f'  {OP_NAMES[op]:>14}: {acc:.1f}% (evaluated live) | d={cfg.d_model} L={cfg.n_layers}')
...
def auto_train(ops=None, target=50.0, budget=50000, rounds=5, deep=False, test_hard=False):
    ...
    if history:
        ...
        for h in history:
            ...
            print(f'    Round {h["round"]}: {op_name} ({h["before_acc"]:.0f}% -> trained {h["steps_used"]:,} steps in {h["time_seconds"]}s)')
...
Examples:
  ...
  python3 auto_train.py --budget 5000       # Quick test budget
    ...
    parser.add_argument('--budget', type=int, default=50000,
                       help='Max total training steps (default: 50000)')
    parser.add_argument('--rounds', type=int, default=5,
                       help='Max re-evaluation rounds (default: 5)')
    parser.add_argument('--deep', action='store_true',
                       help='Use deeper model (d=64 L=8 ff=256)')
    parser.add_argument('--test-hard', action='store_true',
                       help='Also test on max_digits+1')
    parser.add_argument('--ops', nargs='+', default=None,
                       help='Operations to train (default: all)')

...
def train_one( name: str, data_dir: str, save_dir: Path,
               steps=20000, batch_size=512, lr=3e-4):
    ...
    while global_step < steps:
        for x, y in loader:
            ...
            if global_step % max(200, steps // 20) == 0:
                ...
                rate = (len(recent) - 1) / max(0.001, recent[-1] - recent[0]) if len(recent) > 1 else 0
                ...
                print(f'  [{bar}] {global_step:>6d}/{steps} | loss={avg_loss:.4f} | {rate:.1f} st/s | ETA {int(remaining//60):02d}:{int(remaining%60):02d}')
    ...
    if name != 'Generalist':  # only for specialists
        ...
        _save_checkpoint(model, tok, save_dir, steps, max(best_acc, hnm_acc), params, cfg, final=True)
        _export_portable(save_dir, name)
...
def hard_negative_finetune(model, tok, save_dir, num_rounds=3,
                           hard_ratio=0.3, ft_steps=500, lr=1e-4):
...
def main():
    ...
    train_one('Generalist', 'data', OUT / 'generalist',
              steps=20000, batch_size=256, lr=3e-4)
    ...
    for op in ['add', 'sub', 'mul', 'div']:
        ...
        train_one(f'{op} Specialist{" (reversed)" if is_rev else ""}',
                  data_dir, OUT / op,

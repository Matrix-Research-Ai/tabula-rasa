...
def generate_problem(op: str, min_digits=1, max_digits=4, reversed: bool = False, scratchpad: bool = True):
    ...
    def __init__(self, tokenizer, op: str, cfg: Config):
        ...
        for _ in range(max_attempts):
            ...
            if force_carry > 0 and has_carry and carry_count < target_carry:
                ...
                carry_count += 1
            elif force_carry > 0 and not has_carry and (len(self.samples) - carry_count) < (cfg.train_samples - target_carry):
                self.samples.append(ids)
            elif force_carry == 0:
                self.samples.append(ids)
...
def _count_digits(expr: str, op: str) -> int:
    ...
    return max(len(p) for p in expr.replace('+', ' ').replace('-', ' ').replace('*', ' ').replace('/', ' ').split() if p.isdigit())
...
def _validate_config(cfg, op):
    ...
    if cfg.train_samples > 50000 and cfg.max_steps < 20000:
        warnings.append(f'Dataset ({cfg.train_samples}) much larger than steps ({cfg.max_steps}). Consider fewer samples.')

    if cfg.d_model % cfg.n_heads != 0:
        warnings.append(f'd_model ({cfg.d_model}) not divisible by n_heads ({cfg.n_heads}). Training will CRASH.')
...
def train_specialist(op, steps=0, batch_size=0, lr=0,
    ...
    if cfg.gradient_accumulation_steps > 1:
        print(f'  Grad Accum: {cfg.gradient_accumulation_steps} steps (effective batch: {cfg.batch_size * cfg.gradient_accumulation_steps})')
    ...
    print(f'  Building dataset ({cfg.train_samples} samples, max_digits={current_max_digits})...', end=' ', flush=True)
    ...
    print(f'  Reversed: {cfg.use_reversed}  |  Loss masking: {cfg.use_loss_masking}  |  Scratchpad: {cfg.use_scratchpad}')
...
Examples:
  ...
  python3 train_specialist.py add --steps 5000 --batch 64 --lr 0.0005
    ...
    parser.add_argument('op', nargs='?', default='add',
                       help='Operation: add|sub|mul|div|all')
    parser.add_argument('--steps', type=int, default=0,
                       help='Max training steps (0=config default)')
    parser.add_argument('--batch', type=int, default=0,
                       help='Batch size (0=config default)')
    parser.add_argument('--lr', type=float, default=0,
                       help='Learning rate (0=config default)')
    parser.add_argument('--no-reversed', action='store_true',
                       help='Disable reversed digits')
    parser.add_argument('--no-loss-mask', action='store_true',
                       help='Disable loss masking')
    parser.add_argument('--quick', action='store_true',
                       help='Quick smoke test (500 steps, small dataset)')

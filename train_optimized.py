    ...
    for _ in range(bs):
        ...
        if op == 'add': ans = a + b
            ...
            for j in range(len(y)):
                if j < eq or y[j] == tok.pad_id: y[j] = ignore_val
...
def quick_eval(model, tok, cfg, op=None, num=100):
    ...
    for op_n in ops_t:
        for _ in range(num // len(ops_t)):
            e, a = generate_problem(op_n, 1, 4, reversed=(cfg.use_reversed and op_n in ('add', 'sub')))
            out = model.generate(tok, e, max_new_tokens=cfg.eval_max_tokens, temperature=cfg.eval_temperature)
            p = ''.join(c for c in (out.split('=')[-1] if '=' in out else '') if c.isdigit() or c == '-')
            if p == a: correct += 1
...
def train_optimized(op=None, steps=None, batch_size=None):
    ...
    log(f'  Optimizer: {cfg.optimizer}  |  Schedule: {cfg.lr_schedule}  |  Grad clip: {cfg.grad_clip_norm}')
    log('=' * 60)
    ...
    log(f'  Params: {params:,} | d={cfg.d_model} L={cfg.n_layers} ff={cfg.d_ff} | act={cfg.activation} norm={cfg.norm_type} pos={cfg.pos_encoding}')
    ...
    log('')
    while step < steps:
        x, y = make_batch(tok, cfg, op, batch_size, eq_token, cfg.use_reversed, cfg.use_loss_masking)
        _, loss, _ = model(x, y)
        opt.zero_grad()
        loss.backward()
        ...
        if step % max(steps // 10, 100) == 0:
            ...
            log(f'  Step {step:>5d}/{steps} | loss={avg_l:.4f} | {rate:.1f} st/s | ETA {int(rem//60):02d}:{int(rem%60):02d}')
        ...
        if step % max(steps // 5, 100) == 0:
            ...
            torch.save({'model_state_dict': model.state_dict(), 'acc': acc, 'step': step},
    ...
    sd = Path('specialists') / 'math' / (op or 'general')
    sd.mkdir(parents=True, exist_ok=True)
    torch.save({'model_state_dict': model.state_dict(), 'acc': best_acc, 'step': step},
               sd / 'final.pt')
    tok.save(str(sd / 'tokenizer.json'))
    elapsed = time.time() - t0

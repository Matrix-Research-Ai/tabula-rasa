...
def main():
    ...
    parser.add_argument('--mcts-simulations', type=int, default=16, help='MCTS simulations per search')
    parser.add_argument('--lr', type=float, default=1e-4, help='Learning rate')
    args = parser.parse_args()

    print(f'[LanguageAZ] Starting self-play: {args.games} games, {args.difficulty}', flush=True)
    print(f'[LanguageAZ] MCTS={args.mcts} Grammar={args.grammar} ValueHead={args.value_head}', flush=True)
    ...
    print(f'[LanguageAZ] Model created: {sum(p.numel() for p in model.parameters()):,} params', flush=True)

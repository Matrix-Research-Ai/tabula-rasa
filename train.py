...
def train(resume_from=None, resume_steps=None):
    ...
    if resume_from:
        ...
        if ckpt_path.exists():
            ...
            print(f'[*] Resumed from {resume_from} (step {global_step}, acc {best_acc:.1f}%)', flush=True)
        else:
            print(f'[!] Checkpoint not found: {resume_from}. Starting from scratch.')

    if resume_steps:
        cfg.max_steps = global_step + resume_steps
        print(f'[*] Will train for {resume_steps} additional steps (total: {cfg.max_steps})', flush=True)
        ...
        for batch in train_loader:
            ...
            if global_step % cfg.eval_every == 0:
                ...
                if cfg.use_value_head and value_pairs:
                    ...
                    optimizer.step()
                    scheduler.step()
                    print(f'  [*] Value loss: {v_loss.item():.4f} | Target z: {v_targets.mean().item():.2f} avg', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-value-head', action='store_true',
                        help='Enable AlphaZero value head')

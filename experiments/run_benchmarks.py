...
def timeout(seconds: float):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if t.is_alive():
                raise TimeoutError(
                    f'Test exceeded {seconds}s timeout and was aborted'
                )
            if exc:
                raise exc[0]
            return result[0]
...
class BenchmarkRunner:
    ...
    def _run_single(self, name: str, func) -> dict:
        ...
        test_timeout = self.training_thread_timeout if name == 'quick_training' else self.per_test_timeout
        ...
        for carry in range(2):
            for digit in range(10):
                ...
                assert decoded == token_str, (
                    f'Carry token roundtrip: {token_str!r} -> {decoded!r}'
                )

        # Verify carry tokens exist in vocabulary
        carry_count = sum(
            1 for t in tok.stoi if len(t) == 2 and t[0] in '01' and t[1].isdigit()
        )
    ...
    def test_ewc_compute_fisher(self) -> str:
        ...
        assert abs(overlap - 1.0) < 1e-4, (
            f'Same-Fisher overlap should be ~1.0, got {overlap}'
        )
    ...
    def test_api_endpoints(self) -> str:
        ...
        try:
            ...
            health_detail = f'model={health_data.get("model", "?")} params={health_data.get("params", "?")}'
...
def main():
    parser = argparse.ArgumentParser(
        description='Tabula Rasa — Standardised Benchmark Suite'
    )
    parser.add_argument(
        '--quick',
        action='store_true',
